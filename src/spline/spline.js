...
var Spline = Class.extend({
  initialize: function(points) {
    this.points = points;
    this.points.on('change', this.recompile, this);
  },
  /**
  ...
  draw: function draw(ctx, opt) {
    ...
    if(opt.derivatives[0]) {
      ...
      ctx.beginPath();
      for(var i = 0; i < psl - 1; i++) {
        for(var j = 0; j < opt.details; j++) {
          var t = ps[i].t + (ps[i+1].t-ps[i].t)/(opt.details-1)*j;
